import { type User, type InsertUser, type Booking, type InsertBooking, type Document, type InsertDocument, type ParkingSpot, type BusinessInquiry, type InsertBusinessInquiry, type EVStation } from "@shared/schema";
...
class FirebaseStorage implements IStorage {
  ...
  async getUserByPhoneNumber(phoneNumber: string): Promise<User | undefined> {
    try {
      const snapshot = await this.db.collection('users').where('phoneNumber', '==', phoneNumber).limit(1).get();
